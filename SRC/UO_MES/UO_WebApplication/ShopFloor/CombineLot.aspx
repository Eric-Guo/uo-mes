<%@ Page Title="<%$ Resources:Caption %>" Language="C#" MasterPageFile="~/General.Master" AutoEventWireup="true" CodeBehind="CombineLot.aspx.cs" Inherits="UO_WebApplication.ShopFloor.CombineLot" %>
...
<asp:Content ID="Holder1" ContentPlaceHolderID="Holder1" runat="server">
    ...
    <asp:GridView ID="GridView1" runat="server" DataSourceID="ODS_Details" DataKeyNames="ServiceDetailID" AutoGenerateColumns="False" >
        <Columns>
            ...
            <asp:TemplateField>
                <HeaderTemplate><asp:Localize ID="locFromContainerName" runat="server" Text="<%$ Resources:MES,FromContainerName %>" /></HeaderTemplate>
                <ItemTemplate><%# Eval("FromContainerName")%></ItemTemplate>
                <EditItemTemplate>
    <asp:TextBox ID="txtFromContainerName" runat="server" SkinID="container_input" Text='<%# Bind("FromContainerName") %>' />
                </EditItemTemplate>
            </asp:TemplateField>
            <asp:BoundField DataField="Qty" HeaderText="<%$ Resources:MES,Qty %>" SortExpression="Qty" />
            <asp:CheckBoxField DataField="CombineAllQty" HeaderText="<%$ Resources:MES,CombineAllQty %>" SortExpression="CombineAllQty" />
            <asp:CheckBoxField DataField="CloseWhenEmpty" HeaderText="<%$ Resources:MES,CloseWhenEmpty %>" SortExpression="CloseWhenEmpty" />
        </Columns>
    </asp:GridView>
    <asp:ObjectDataSource ID="ODS_Details" runat="server" 
        TypeName="UO_Service.ContainerTxn.Combine" SelectMethod="SelectDetails" 
        DataObjectTypeName="UO_Service.ContainerTxn.CombineFromDetail"
...
<td><asp:Button ID="btnCombine" runat="server" Text="<%$ Resources:Execute %>" onclick="btnCombine_Click"/></td>
        </tr>
        <tr>
